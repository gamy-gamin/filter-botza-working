    ...
    for file in query:
        if file['file_size'] != "👍":
            try:
              if file['file_name'].index("srt")> 0:
                  filename = SUB_TEXT + file['file_name']
                  filelink = file['link']+"?"+file['file_name']+"?size?"+str(file['file_size']//1048576) + "MB"
                  filenames.append(filename)
...
async def searchquery_link(group_id, name):
    ...
    for file in query:
        if file['file_size'] == "👍":
            ...
            filenames.append(filename)
            filelinks.append(filelink.replace(" ", ".").replace(CHANNEL_LINK, WEB_SITE_URL).replace("https://t.me/c/1404974777/", WEB_SITE_URL))
    return filenames, filelinks
